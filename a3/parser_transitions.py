...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        temp_bs = batch_size if len(unfinished_parses) >= batch_size else len(unfinished_parses)
        transitions = model.predict(unfinished_parses[:temp_bs])
        for i in range(temp_bs):
            unfinished_parses[i].parse_step(transitions[i])
        unfinished_parses = list(filter(lambda x: not(len(x.buffer) == 0 and len(x.stack) == 1), unfinished_parses))
