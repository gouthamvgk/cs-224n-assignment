...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTM(embed_size+self.hidden_size, self.hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = last_hidden.reshape(-1, 2*self.hidden_size)
        init_decoder_hidden = self.h_projection(last_hidden)
        last_cell = torch.transpose(last_cell, 0,1)
        last_cell = last_cell.reshape(-1, 2*self.hidden_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = (dec_state[0].unsqueeze(0), dec_state[1].unsqueeze(0))
        _, dec_state = self.decoder(Ybar_t.unsqueeze(0),dec_state )
